...
/// Safe file/URL identities survive restart; legacy base64 stays process-local.
double? avatarRatio(String avatar) => imageRatioCache.ratioOf(avatar);
...
void noteAvatarRatio(String avatar, double ratio) =>
    imageRatioCache.note(avatar, ratio);

/// Drops every cached provider and intrinsic ratio. For tests and for the
/// "free up memory" path; durable storage is removed by [Storage.clearCache].
void clearAvatarImageCache() {
  _cache.clear();
  _bytes.clear();
  imageRatioCache.reset();
}
